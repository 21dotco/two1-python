...
# Run Quickest first
### Clean Up Base Image ###
inform "Purging unneeded dependencies"
sudo rm -r ~/python_games
sudo apt-get purge -y wolfram-engine minecraft-pi sonic-pi

inform "Updating keyboard configuration..."
sudo cp ~/static/keyboard /etc/default/keyboard
...
sudo pip3.4 install cython -v || abort 'Failed to install cython'

# Install Numpy
inform "Installing numpy"
sudo apt-get install -y --force-yes python-numpy || abort 'Failed to install numpy'
...
sudo pip3.4 install scipy -v || abort 'Failed to install scipy'
...
# Clean Up
inform "Cleaning up build..."
sudo rm -r static get-pip.py || abort 'Failed to remove get-pip.py'
