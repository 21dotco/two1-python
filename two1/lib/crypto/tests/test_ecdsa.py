...
from two1.lib.crypto.ecdsa import ECPointAffine, ECPointJacobian, secp256k1, p256
...
def test_ecpoint(curve, point_type):
    ...
    if point_type == 'affine':
        base_point = ECPointAffine(curve, curve.Gx, curve.Gy)
    elif point_type == 'jacobian':
        base_point = ECPointJacobian(curve, curve.Gx, curve.Gy, 1)
    else:
