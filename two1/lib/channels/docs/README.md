...
![Payment Channels Protocol](https://github.com/21dotco/two1/blob/devel/two1/lib/channels/docs/PCs.png)
...
Each payment channel is modeled by a state machine `PaymentChannelStateMachine` class ([two1/lib/channels/statemachine.py](https://github.com/21dotco/two1/blob/devel/two1/lib/channels/statemachine.py)), which provides an interface to manipulating all of the client-side channel state. This state is stored in a `PaymentChannelModel` ([two1/lib/channels/statemachine.py](https://github.com/21dotco/two1/blob/devel/two1/lib/channels/statemachine.py)) object, which can be stored and restored to and from the channels database. The `PaymentChannelStateMachine` class provides the low-level transition functions on the channel state and is responsible for creating and returning the underlying refund, deposit, and payment transactions. It uses a `WalletWrapper` class ([two1/lib/channels/wallet.py](https://github.com/21dotco/two1/blob/devel/two1/lib/channels/wallet.py)) to sign transactions, but otherwise has no interaction with the outside world.

The `PaymentChannel` class ([two1/lib/channels/paymentchannel.py](https://github.com/21dotco/two1/blob/devel/two1/lib/channels/paymentchannel.py])) provides an internal API (open, pay, sync, close, and properties) to a payment channel, operates transitions on the state machine, and is the the glue between state machine and the outside world -- the database [two1/lib/channels/database.py](https://github.com/21dotco/two1/blob/devel/two1/lib/channels/database.py), the blockchain [two1/lib/channels/blockchain.py](https://github.com/21dotco/two1/blob/devel/two1/lib/channels/blockchain.py), and the payment channel server [two1/lib/channels/server.py](https://github.com/21dotco/two1/blob/devel/two1/lib/channels/server.py). Its logic is described in [two1/lib/channels/docs/channel-logic.txt](https://github.com/21dotco/two1/blob/devel/two1/lib/channels/docs/channel-logic.txt).

The `PaymentChannelClient` class ([two1/lib/channels/paymentchannelclient.py](https://github.com/21dotco/two1/blob/devel/two1/lib/channels/paymentchannelclient.py])) provides the top-level API for applications to `open()`, `pay()`, `sync()`, `status()`, `close()`, and `list()` payment channels by  URL.

Finally, the `channels` cli is click-based cli implemented in [two1/lib/channels/cli.py](https://github.com/21dotco/two1/blob/devel/two1/lib/channels/cli.py).
