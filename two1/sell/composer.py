...
class Two1ComposerContainers(Two1Composer):
    ...
    @staticmethod
    def service_names_from_container_names(containers):
        ...
        Args:
            containers (list): List of container descriptions as returned by self.docker_client.containers.

        Returns:
            set: Set of service names generated by removing the 'sell_' prefix from the containers' names.
        """
        return set([service['Names'][0][6:] for service in containers])
    ...
    def status_services(self, services):
        ...
        existent_services = self.service_names_from_container_names(
            self.docker_client.containers(all=True)
        ) - Two1Composer.BASE_SERVICES
        running_services = self.get_running_services()
        exited_services = self.service_names_from_container_names(
            self.docker_client.containers(filters={"status": "exited"})
        ) - Two1Composer.BASE_SERVICES
    ...
    def get_running_services(self):
        ...
        """
        running_containers = self.docker_client.containers(filters={"status": "running"})
        return self.service_names_from_container_names(running_containers) - Two1Composer.BASE_SERVICES
