...
def _handle_402_response(wallet, res, req, max_request_price=DEFAULT_MAX_REQUEST_PRICE, goal_price=0, logger=None):
	...
	amount = _get_price_for_goal(res.headers['Price'], goal_price=goal_price, logger=logger)
	address = str(res.headers['Bitcoin-Address'])
	_conditional_log( "402: Server requested " + str(amount) + " satoshi be paid to " + address, logger )

	# Check price is within our bounds
	if amount == 0:
		_conditional_log( "Aborting payment: Requested amount is 0", logger )
		return req

	# If the 402 requested amount exceeds the set maximum amount then abort.
	if amount > max_request_price:
		_conditional_log( 'Aborting payment: Amount exceeds max_request_price. (' + str(amount) + ' > ' + str(max_request_price) + ' ).', logger)
	...
	if 'Price-Token' in res.headers:
		...
		_conditional_log( "402: Persisted price token '" + req['headers']['Price-Token'] + "'", logger )
...
def _retrieve_paid_asset( req, delay=DEFAULT_BASE_BACKOFF_DELAY, backoff_max_delay=DEFAULT_MAX_RETRY_DELAY, logger=None ):
	...
	if newDelay >=  backoff_max_delay:
		...
		_conditional_log( "Aborting: The request delay exceeded max backoff delay.", logger)
...
def _get_price_for_goal(price_object, goal_price=0, logger=None):
	...
	if isinstance(price_object, int):
		_conditional_log( "402: Has fixed price of " + str(price_object) + " satoshi", logger)
		return price_object

	# Validate that it is supported quote object.
	if not isinstance(price_object, dict):
		_conditional_log( "Aborting: Price object is unsupported, got '" + str(price_object) + "' expected 'dict' or 'int'", logger)
	...
	if 'min' not in price_object and 'max' not in price_object:
		_conditional_log( "Aborting: Price object did not define any supported rules.", logger)
		raise ValueError("A price object must have one or more rules.");

	# Get the current values.
	if 'min' not in price_object:
		_conditional_log( "402: Price object defined max payment amount to " + str(price_object['max']) + " satoshi", logger)
		return min( goal_price, price_object['max'] );
	if 'max' not in price_object:
		_conditional_log( "402: Price object defined min payment amount to " + str(price_object['min']) + " satoshi", logger)
		return max( goal_price, price_object['min'] );

	# Both exist
	max_v = price_object['max']
	min_v = price_object['min']
	_conditional_log( "402: Price object defined payment range of ( min: " + str(min_v) + ", max: " + str(max_v) + " )", logger )
	if min > max:
		_conditional_log( "Aborting: Price object rule conflict.", logger );
...
def bitcurl(
	...
	if res.status_code != 402:
		_conditional_log( "Aborting payment: Server responded with " +  str(res.status_code), logger)
		return res

	# Make payment and update request
	_conditional_log( "402: Was received, will attempt to pay.", logger );
	req = _handle_402_response( wallet, res, req , max_request_price=max_request_price, goal_price=goal_price, logger=logger)

	# Get the final response
	finalRes = _retrieve_paid_asset( req, delay=backoff_delay, backoff_max_delay=backoff_max_delay, logger=logger )
