...
def PaymentRequiredExceptionHandler(exc, context):
    # Get the default exeption response from the rest framework.
...
class PaymentRequiredAuthentication(authentication.BaseAuthentication):
    ...
    # This can be overriden by a subclass to support dynamic prices.
    ...
    # This can be overriden by a subclass to support generated addresses
    # NOTE: this method should be 100% deterministic or transaction
    # verification will fail.
    ...
    def getTransactionFor(self, request):
        ...
        # 2. Body
        elif "txId" in request.POST:
            return request.POST["txId"]

        # 3. Query
        elif "txId" in request.GET:
            return request.GET["txId"]

        elif "tx" in request.POST:
            return request.POST["tx"]
